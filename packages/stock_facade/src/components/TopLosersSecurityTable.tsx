...
export async function getTopLosers(source: CancelTokenSource): Promise<any> {
...
export const TopLosersSecurityTable = React.memo(
...
export const TopLosersSecurityView = () => {
  ...
  const fetchTopLosers = async (source: CancelTokenSource) => {
    try {
      setData({ ...data, isFetching: true });
      const response = await getTopLosers(source);
      setData({
  ...
  useEffect(() => {
    let mounted = true;
    if (mounted) {
      fetchTopLosers(cancelTokenSource);
    }
      ...
      <HeaderSplitContainer>
        ...
        <HeaderGroup style={{ textAlign: "right" }}>
          <Button
            ...
            onClick={() => fetchTopLosers(cancelTokenSource)}
          />
        </HeaderGroup>
      </HeaderSplitContainer>
      <Divider />
